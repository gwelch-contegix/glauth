...
RUN mkdir -p /root/v2/ && cd /root/v2/ && go mod init temp && go get github.com/glauth/glauth/v2@master && go mod download && go build -ldflags "-s -w" -v -buildvcs=false -trimpath github.com/glauth/glauth/v2 || true
COPY / /root/rpmbuild
